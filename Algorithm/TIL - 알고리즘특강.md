...
- 검색 : 정렬된 상태에서 빠르게 원하는 것을 찾을 수 있음
  - 순차 검색 : 검색할 집합이 정렬되어 있지 않은 상태, 순서대로 처음부터 끝까지 검색
  - **이진 검색 : 정렬이 된 데이터에서 반씩 잘라내며 한쪽을 버리는 방식**
  - 트리 검색 : 데이터 검색시 효율적이지만, 삽입, 삭제시 부담됨

## 재귀호출

- 재귀 알고리즘 : 동일한 작동을 무한적으로 반복하는 알고리즘
  - 재귀호출은 재귀호출로 부르는 만큼 함수가 열린다고 생각!
